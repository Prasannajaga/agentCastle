...
export interface RunOptions {
  ...
  readonly cwd?: string;
  /**
   * Sandcastle config directory. Defaults to `<cwd>/.sandcastle`.
   * Use this when the repo being edited should be separate from the
   * Sandcastle config, logs, worktrees, env file, and patch artifacts.
   */
  ...
  /**
   * Reuse one sandbox across all iterations instead of recreating per iteration.
   * Defaults to `true` for OpenCode and `false` for other providers.
   */
...
export const run = async (options: RunOptions): Promise<RunResult> => {
  ...
  const reuseSandboxAcrossIterations =
    options.reuseSandboxAcrossIterations ?? provider.name === "opencode";

  // Resolve env vars and merge with provider env
  const resolvedEnv = await Effect.runPromise(
    resolveEnv(hostRepoDir, configDir).pipe(Effect.provide(NodeContext.layer)),
  );
  ...
  const factoryLayer = Layer.provide(
    ...
    Layer.mergeAll(
      Layer.succeed(SandboxConfig, {
        env,
        hostRepoDir,
        configDir: options.configDir ? configDir : undefined,
