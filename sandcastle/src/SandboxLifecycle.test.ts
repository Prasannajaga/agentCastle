...
describe("withSandboxLifecycle (worktree mode)", () => {
  ...
  const setupEmptyRepoWorktree = async () => {
    const hostDir = await mkdtemp(join(tmpdir(), "host-empty-"));
    await execAsync("git init -b main", { cwd: hostDir });
    await execAsync('git config user.email "test@test.com"', { cwd: hostDir });
    await execAsync('git config user.name "Test"', { cwd: hostDir });
    const layer = makeLocalSandboxLayer(hostDir);
    return { hostDir, worktreeDir: hostDir, layer };
  };
  ...
  it("handles unborn HEAD gracefully when explicit branch has no commits", async () => {
    const { hostDir, worktreeDir, layer } = await setupEmptyRepoWorktree();
  ...
  it("collects first commit on an unborn explicit branch", async () => {
    const { hostDir, worktreeDir, layer } = await setupEmptyRepoWorktree();
    ...
    const result = await Effect.runPromise(
      withSandboxLifecycle(
        ...
        (ctx) =>
          Effect.gen(function* () {
            yield* ctx.sandbox.exec(
              'sh -c "echo first > first.txt && git add first.txt && git commit -m \\"first commit\\""',
              { cwd: ctx.sandboxRepoDir },
            );
          }),
      ).pipe(Effect.provide(Layer.merge(layer, testDisplayLayer))),
    ...
    expect(result.commits).toHaveLength(1);
    expect(result.branch).toBe("main");
    const file = await readFile(join(hostDir, "first.txt"), "utf-8");
    expect(file.trim()).toBe("first");
  });
