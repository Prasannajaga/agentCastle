...
const OUTPUT_TAIL_MAX_LINE_CHARS = 220;

const formatCompletionSignals = (signals: readonly string[]): string =>
  signals.map((sig) => JSON.stringify(sig)).join(" OR ");

const extractOutputTailLines = (
  output: string,
  ...
  if (lines.length === 0) {
    ...
    return [
      compact.length > OUTPUT_TAIL_MAX_LINE_CHARS
        ? `${compact.slice(0, OUTPUT_TAIL_MAX_LINE_CHARS)}...`
        : compact,
    ];
  }
  return lines.slice(-maxLines).map((line) =>
    line.length > OUTPUT_TAIL_MAX_LINE_CHARS
      ? `${line.slice(0, OUTPUT_TAIL_MAX_LINE_CHARS)}...`
      : line,
  );
};
...
export interface OrchestrateOptions {
  ...
  /** When true, create one sandbox for the full run and reuse it across all iterations. */
  readonly reuseSandboxAcrossIterations?: boolean;
}
...
> => {
  ...
  return Effect.gen(function* () {
    ...
    const reuseSandboxAcrossIterations =
      options.reuseSandboxAcrossIterations ?? false;
    ...
    const runIteration = (
      i: number,
      ctx: { sandbox: SandboxService; sandboxRepoDir: string },
      ...
      Effect.gen(function* () {
        ...
        if (iterationResumeSession && bindMountHandle) {
          ...
          yield* Effect.tryPromise({
            try: () => transferSession(hStore, sbStore, iterationResumeSession),
            catch: (e) =>
        ...
        const fullPrompt = options.skipPromptExpansion
          ? prompt
          : yield* preprocessPrompt(prompt, ctx.sandbox, ctx.sandboxRepoDir);
        ...
        if (matchedSignal !== undefined) {
          yield* display.status(
            label(
              `Agent stopped: completion signal matched (${matchedSignal}).`,
            ),
            "success",
          );
        } else {
          yield* display.status(
            label(
              "Agent stopped: iteration finished without completion signal.",
            ),
            "warn",
          );
          yield* display.status(
            label(
              `Expected completion signal(s): ${formatCompletionSignals(completionSignals)}.`,
            ),
            "info",
          );
          const outputTail = extractOutputTailLines(agentOutput);
          if (outputTail.length === 0) {
            yield* display.status(
              label("Agent output tail: <empty output from agent process>."),
              "warn",
            );
          } else {
            yield* display.status(
              label(
                `Agent output tail (${outputTail.length} line${outputTail.length === 1 ? "" : "s"}):`,
              ),
              "info",
            );
            for (const line of outputTail) {
              yield* display.status(label(`  ${line}`), "info");
            }
          }
        }
    ...
    if (reuseSandboxAcrossIterations) {
            ...
            (ctx) =>
              Effect.gen(function* () {
                ...
                for (let i = 1; i <= iterations; i++) {
                  ...
                  allIterations.push({
                    sessionId: iterationResult.sessionId,
                    sessionFilePath: iterationResult.sessionFilePath,
      ...
      const sandboxResult = yield* factory.withSandbox(
        ({ hostWorktreePath, sandboxRepoPath, applyToHost, bindMountHandle }) =>
          withSandboxLifecycle(
            ...
            (ctx) => runIteration(i, ctx, bindMountHandle),
          ),
      );
