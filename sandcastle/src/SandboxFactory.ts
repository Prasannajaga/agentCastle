...
export class SandboxConfig extends Context.Tag("SandboxConfig")<
  SandboxConfig,
  {
    ...
    /** Directory containing Sandcastle config, logs, worktrees, and patches. Defaults to <hostRepoDir>/.sandcastle. */
...
const cleanupWorktree = (
  ...
  WorktreeManager.hasUncommittedChanges(worktreePath).pipe(
    ...
    Effect.flatMap((isDirty) => {
      ...
      const remove = repoDir
        ? WorktreeManager.remove(worktreePath, repoDir)
        : WorktreeManager.remove(worktreePath);
      return remove.pipe(Effect.map(() => undefined as string | undefined));
    }),
  );
  ...
  layer: Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const pruneAndCreate = () => {
        const prune = configDir
          ? WorktreeManager.pruneStale(hostRepoDir, configDir)
          : WorktreeManager.pruneStale(hostRepoDir);
        const createOptions = branch
          ? {
              branch,
              ...(baseBranch !== undefined || sandboxProvider.tag !== "isolated"
                ? { baseBranch }
                : {}),
              ...(configDir ? { configDir } : {}),
            }
        ...
        return prune.pipe(
          ...
          Effect.andThen(
            branch
              ? WorktreeManager.create(hostRepoDir, createOptions)
              : WorktreeManager.create(hostRepoDir, createOptions),
          ),
                ...
                Effect.tryPromise({
                  ...
                  Effect.andThen(
                    cleanupWorktree(
                      worktreeInfo.path,
                      exit,
                      configDir ? hostRepoDir : undefined,
                    ),
                  ),
                  Effect.tap((p) => {
                    preservedPath = p;
