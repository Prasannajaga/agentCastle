...
describe("opencode factory", () => {
  ...
  it("parseStreamLine extracts text from JSON with .text property", () => {
    const provider = opencode("opencode/big-pickle");
    expect(
  ...
  it("parseStreamLine extracts text from JSON with .message property", () => {
    const provider = opencode("opencode/big-pickle");
    expect(
      provider.parseStreamLine(JSON.stringify({ message: "hello there" })),
  ...
  it("parseStreamLine returns empty array when error event has no extractable message", () => {
    const provider = opencode("opencode/big-pickle");
    const rawLine = JSON.stringify({
      type: "error",
      code: "unknown",
    });
    expect(provider.parseStreamLine(rawLine)).toEqual([]);
  });

  it("parseStreamLine forwards raw text when input is not valid JSON", () => {
    const provider = opencode("opencode/big-pickle");
  ...
  it("parseStreamLine forwards raw text when JSON does not contain text or message", () => {
    const provider = opencode("opencode/big-pickle");
    const rawLine = '{"foo": "bar"}';
    expect(provider.parseStreamLine(rawLine)).toEqual([
      { type: "text", text: rawLine },
    ]);
  });

  it("parseStreamLine forwards raw text when JSON text/message properties are empty strings", () => {
    const provider = opencode("opencode/big-pickle");
    const line1 = JSON.stringify({ text: "   " });
    expect(provider.parseStreamLine(line1)).toEqual([
      { type: "text", text: line1 },
    ]);
    const line2 = JSON.stringify({ message: "" });
    expect(provider.parseStreamLine(line2)).toEqual([
      { type: "text", text: line2 },
    ]);
  });

  it("parseStreamLine forwards raw text when JSON is malformed", () => {
    const provider = opencode("opencode/big-pickle");
    expect(provider.parseStreamLine("{bad json")).toEqual([
  ...
  it("parseStreamLine returns empty array for empty strings", () => {
    const provider = opencode("opencode/big-pickle");
    expect(provider.parseStreamLine("")).toEqual([]);
    expect(provider.parseStreamLine("   ")).toEqual([]);
  });
