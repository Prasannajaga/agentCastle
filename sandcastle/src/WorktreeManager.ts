...
const canResolveCommit = (
  repoDir: string,
  ref: string,
): Effect.Effect<boolean, never> =>
  execGit(["rev-parse", "--verify", `${ref}^{commit}`], repoDir).pipe(
    Effect.as(true),
...
export const getCurrentBranch = (
  ...
  execGit(["branch", "--show-current"], repoDir).pipe(
    Effect.map((output) => output.trim()),
    Effect.map((branch) => (branch.length > 0 ? branch : "HEAD")),
  );
...
> =>
  Effect.gen(function* () {
    ...
    const configDir = opts?.configDir ?? join(repoDir, ".sandcastle");
    const worktreesDir = join(configDir, "worktrees");
    yield* fs
    ...
    if (opts?.branch) {
        ...
        Effect.catchAll((e) => {
          if (e.message.includes("invalid reference")) {
            return Effect.gen(function* () {
              const baseRef = opts?.baseBranch ?? "HEAD";
              const baseResolvable = yield* canResolveCommit(repoDir, baseRef);
              if (baseResolvable) {
      ...
      const headResolvable = yield* canResolveCommit(repoDir, "HEAD");
      const addArgs = headResolvable
        ? [
            ...NO_CONFIG_LOCK_FLAGS,
            "worktree",
            "add",
      ...
      yield* execGit(addArgs, repoDir).pipe(
        Effect.catchAll((e) => {
          if (
...
export const remove = (
  ...
  const gitRepoDir = repoDir ?? join(worktreePath, "..", "..", "..");
  return execGit(
...
export const pruneStale = (
  repoDir: string,
  configDir = join(repoDir, ".sandcastle"),
): Effect.Effect<
  void,
  WorktreeError | WorktreeTimeoutError,
  FileSystem.FileSystem
