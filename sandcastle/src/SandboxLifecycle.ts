...
const getCurrentBranchFromHost = (
  cwd: string,
...
export interface SandboxContext {
  ...
  readonly sandboxRepoDir: string;
  /** Empty when the host branch/worktree has no commits yet (unborn HEAD). */
...
export const withSandboxLifecycle = <A>(
  ...
  Effect.gen(function* () {
    ...
    const hostCurrentBranch: string | null = !branch
      ? yield* getCurrentBranchFromHost(hostRepoDir)
      : null;
    ...
    yield* display.taskLog("Setting up sandbox", (message) =>
      Effect.gen(function* () {
        ...
        if (resolvedBranch.length === 0) resolvedBranch = "HEAD";
    ...
    const baseHead: string | null = yield* Effect.promise(async () => {
      try {
        ...
        Effect.promise(async () => {
          try {
            const listCommand = baseHead
              ...
              : `git rev-list "refs/heads/${targetBranch}" --reverse`;
            const { stdout } = await execAsync(
              listCommand,
              { cwd: hostRepoDir },
            );
