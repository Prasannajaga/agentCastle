...
describe("Orchestrator", () => {
  ...
  it("reuses a single sandbox across iterations when enabled", async () => {
    const hostDir = await mkdtemp(join(tmpdir(), "orch-reuse-host-"));
    ...
    const result = await Effect.runPromise(
      orchestrate({
        ...
        iterations: 3,
        prompt: "test reuse",
        reuseSandboxAcrossIterations: true,
      }).pipe(Effect.provide(Layer.merge(factoryLayer, testDisplayLayer))),
    );
