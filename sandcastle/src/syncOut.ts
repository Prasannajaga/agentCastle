...
const createPatchDir = (
  hostRepoDir: string,
  configDir = join(hostRepoDir, ".sandcastle"),
): Effect.Effect<string, SyncError> =>
  Effect.tryPromise({
    try: async () => {
      const now = new Date();
      ...
      const patchesRoot = join(configDir, "patches");
...
export const syncOut = (
  ...
  Effect.gen(function* () {
    ...
    const patchDir = yield* createPatchDir(hostRepoDir, configDir);
    const relativePatchDir =
      configDir === join(hostRepoDir, ".sandcastle")
