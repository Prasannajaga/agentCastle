...
describe("WorktreeManager.create", () => {
  ...
  it("creates a temp worktree on an empty repo with unborn HEAD", async () => {
    const repoDir = await setupEmptyRepo();
    const { path, branch } = await run(create(repoDir));
  ...
  it("creates a specified branch worktree on an empty repo with unborn HEAD", async () => {
    const repoDir = await setupEmptyRepo();
    const { path, branch } = await run(
      create(repoDir, { branch: "test-unborn-branch" }),
    );

    expect(branch).toBe("test-unborn-branch");
    expect(await getBranch(path)).toBe("test-unborn-branch");
...
describe("getCurrentBranch", () => {
  it("returns the branch name when HEAD is unborn", async () => {
    const repoDir = await setupEmptyRepo();
