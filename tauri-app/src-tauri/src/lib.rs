...
const agentFactory = agentProviderName === "codex"
  ? codex
  : agentProviderName === "opencode"
    ? opencode
    : agentProviderName === "pi"
      ? pi
      : claudeCode;
...
const result = await run({{
  ...
  branchStrategy: {{
    type: "branch",
    branch: required("SANDCASTLE_BRANCH"),
  }},
  ...
  logging: {{
    ...
    onAgentStreamEvent(event) {{
      console.log("**SANDCASTLE_STREAM**=" + JSON.stringify(event));
    }},
  }},
...
async fn cleanup_managed_worktree_after_failure(
    ...
    let derived_worktree = PathBuf::from(&agent.directory)
        .join(".sandcastle")
        .join("worktrees")
        .join(agent.branch.replace('/', "-"));
    candidates.push(derived_worktree);
    ...
    for worktree_path in candidates {
        ...
        if !status.trim().is_empty() {
            notes.push(format!(
                "Preserved dirty worktree at {}. Review or remove it manually after saving any useful changes.",
                worktree_path.display()
            ));
            continue;
        }
        ...
        let remove_result = git(&target_repo, &["worktree", "remove", &worktree_arg]).await;
    ...
    let cleanup_path = run_dir.join("worktree-cleanup.json");
    let payload = json!({
        "timestamp": now_iso(),
        "notes": notes,
    });
    let _ = write_json_atomic(&cleanup_path, &payload).await;
    Some(notes.join(" "))
}
